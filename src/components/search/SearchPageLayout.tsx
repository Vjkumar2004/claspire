...
const FILTERS = [
  { id: 'all', label: 'All Results', icon: Compass, path: '' },
  { id: 'people', label: 'People', icon: Users, path: '/people' },
  { id: 'jobs', label: 'Jobs', icon: Briefcase, path: '/jobs' },
  { id: 'communities', label: 'Communities', icon: Building2, path: '/communities' },
  { id: 'colleges', label: 'Colleges', icon: GraduationCap, path: '/colleges' },
...
function SearchPageContent({ activeFilter }: SearchPageLayoutProps) {
  ...
  // Trigger search on query or route filter changes
  useEffect(() => {
    setOffset(0)
  ...
  const handleTabChange = (filterId: string) => {
    const matched = FILTERS.find(f => f.id === filterId)
    if (!matched) return
    router.push(`/search${matched.path}?q=${encodeURIComponent(queryParam)}`)
  }
          ...
          <div className="hidden lg:block lg:col-span-3 lg:sticky lg:top-20 space-y-4">
              ...
              <nav className="space-y-1">
                {FILTERS.map((f) => {
                  ...
                  return (
                    ...
                    >
                      <f.icon className={`w-4 h-4 ${isActive ? 'text-[#0A66C2]' : 'text-gray-400'}`} />
                      <span>{f.label}</span>
                    </button>
                  )
                })}
            ...
            <div className="bg-surface border border-surface rounded-md p-4 shadow-[0_1px_3px_rgba(0,0,0,0.05)] hidden lg:block">
              <div className="flex gap-2.5 items-start">
                <Info className="w-4 h-4 text-[#0A66C2] mt-0.5 flex-shrink-0" />
                <div>
          ...
          <div className="lg:col-span-6 space-y-4 w-full">
            ...
            <div className="bg-surface border border-surface rounded-md p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
              <h1 className="text-base sm:text-lg font-bold text-gray-900 leading-tight">
                Search results for "<span className="text-[#0A66C2]">{queryParam || 'empty'}</span>"
              </h1>
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full border text-xs font-bold whitespace-nowrap transition-all flex-shrink-0 cursor-pointer ${
                      isActive
                        ? 'bg-[#0A66C2] border-transparent text-white shadow-sm'
            ...
            {loading ? (
                ...
                {(() => {
                  ...
                  return results.map((card, idx) => {
                    ...
                    if (idx === lastSeniorIdx && activeFilter === 'all') {
                      viewMoreSeniorsCard = (
                        <div className="bg-white border border-[#0A66C2]/20 rounded-md p-3.5 text-center shadow-[0_1px_3px_rgba(124,58,237,0.02)] mt-2 mb-4">
                          ...
                          <button
                            ...
                            className="mt-2 px-4 py-1.5 bg-[#0A66C2] hover:bg-[#004182] text-white font-bold text-xs rounded-full shadow-sm cursor-pointer transition-all hover:shadow"
                          >
              ...
              <div className="bg-surface border border-surface rounded-md p-8 sm:p-12 shadow-[0_1px_3px_rgba(0,0,0,0.05)] text-center">
                <div className="w-16 h-16 rounded-full bg-[#EAF4FF] border border-[#0A66C2]/20 flex items-center justify-center mx-auto mb-5">
                  <Compass className="w-8 h-8 text-[#0A66C2]" />
                </div>
                ...
                <p className="text-gray-500 text-xs leading-relaxed max-w-sm mx-auto mt-2 font-semibold">
                  We couldn't find matches for "{queryParam || 'empty'}". Try refining your search filters, checking your spelling, or exploring broader keywords.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-surface border border-surface rounded-md p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
              ...
              <div className="space-y-3">
                {trendingJobs.map((rec, i) => (
                  <div key={i} className="group cursor-pointer" onClick={() => router.push('/jobs')}>
                    <h4 className="text-xs font-bold text-gray-800 group-hover:text-[#0A66C2] transition-colors leading-tight">
                      {rec.role}
                    </h4>
              ...
              <div className="mt-4 pt-3.5 border-t border-surface">
                <Link href="/jobs" className="text-[11px] font-bold text-[#0A66C2] hover:text-[#004182] hover:underline no-underline block text-center">
                  Explore Placements →
                </Link>
              </div>
            </div>
...
export default function SearchPageLayout({ activeFilter }: SearchPageLayoutProps) {
  return (
    <Suspense fallback={
