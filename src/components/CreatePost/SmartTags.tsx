  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative group">
        <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-500/20 to-cyan-500/20 rounded-xl blur opacity-0 group-focus-within:opacity-100 transition duration-500"></div>
        <div className="relative flex flex-wrap items-center gap-2 p-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl backdrop-blur-xl transition-colors group-focus-within:border-purple-500/50">
          {tags.map(tag => (
            <span
              key={tag}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-200 text-sm font-medium rounded-lg border border-purple-200 dark:border-purple-500/30"
            >
              #{tag}
              <button
                type="button"
                onClick={() => onRemoveTag(tag)}
                className="hover:bg-purple-500/30 p-0.5 rounded-md transition-colors"
              >
                <X size={12} />
              </button>
            </span>
