          ...
          <div className="px-8 py-6 border-b border-slate-200 dark:border-white/10 flex items-center justify-between bg-slate-50 dark:bg-white/[0.02] backdrop-blur-xl">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="bg-[#EAF4FF] dark:bg-purple-500/20 text-[#004182] dark:text-purple-300 px-3 py-1 rounded-full text-xs font-bold border border-[#0A66C2]/30 dark:border-purple-500/30">
            ...
            {context.canPostAsCollege && (
              <div className="flex items-center gap-2 bg-slate-100 dark:bg-white/5 p-1 rounded-xl border border-slate-200 dark:border-white/10">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${data.isCollegePost ? 'bg-[#EAF4FF] text-[#004182] dark:bg-purple-500/20 dark:text-purple-300' : 'text-slate-500 hover:text-slate-900 dark:text-white/50 dark:hover:text-white'}`}
                >
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar p-8 space-y-8">
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all duration-300 group ${
                        isSelected 
                          ? 'border-[#0A66C2] bg-[#EAF4FF] dark:bg-purple-500/10 shadow-[0_0_30px_rgba(139,92,246,0.15)] -translate-y-1' 
                    ...
                    >
                      ...
                      <span className={`text-xs font-bold ${isSelected ? 'text-[#004182] dark:text-purple-300' : 'text-slate-600 dark:text-white/70'}`}>{t.label}</span>
                      {isSelected && (
                        <div className="absolute top-2 right-2 w-4 h-4 rounded-full bg-[#0A66C2] flex items-center justify-center text-white">
                          <Check size={10} />
                        </div>
                      )}
                    </button>
            ...
            <div className="space-y-2">
              <input
                ...
                maxLength={120}
                className="w-full bg-transparent border-b-2 border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 focus:border-[#0A66C2] py-4 text-3xl font-bold text-slate-900 dark:text-white placeholder-slate-300 dark:placeholder-white/20 outline-none transition-colors"
              />
                ...
                <button
                  ...
                  className={`flex items-start gap-3 p-4 rounded-2xl border text-left transition-all ${
                    data.visibility === 'private'
                      ? 'border-[#0A66C2]/50 bg-[#EAF4FF] dark:bg-purple-500/10'
                      : 'border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5 hover:border-slate-300 dark:hover:border-white/20'
                  }`}
                >
                  <Lock className={`mt-0.5 ${data.visibility === 'private' ? 'text-[#0A66C2] dark:text-purple-400' : 'text-slate-400 dark:text-white/50'}`} size={20} />
                  <div>
          ...
          <div className="p-6 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] backdrop-blur-xl flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className="px-8 py-3 bg-[#0A66C2] hover:bg-[#004182] text-white rounded-xl font-bold shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:shadow-[0_0_30px_rgba(139,92,246,0.5)] transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:-translate-y-0.5 active:translate-y-0 flex items-center gap-2"
              >
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar p-8">
            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[24px] p-6 shadow-xl">
              ...
              <div className="flex items-start gap-4 mb-4">
                <div className="w-12 h-12 rounded-full bg-[#0A66C2] flex items-center justify-center overflow-hidden shrink-0">
              ...
              {/* Content Preview */}
              <div className="text-sm text-slate-700 dark:text-white/70 break-words opacity-80 line-clamp-6 [&>blockquote]:border-l-4 [&>blockquote]:border-[#0A66C2] [&>blockquote]:pl-4 [&>blockquote]:text-slate-500 dark:[&>blockquote]:text-white/70 [&>ul]:list-disc [&>ul]:pl-5 [&>ol]:list-decimal [&>ol]:pl-5 [&>a]:text-cyan-500 dark:[&>a]:text-cyan-400 [&>a]:underline">
                {data.content ? (
              ...
              {data.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {data.tags.map(t => (
                    <span key={t} className="text-xs text-[#004182] bg-[#EAF4FF] dark:text-purple-300 dark:bg-purple-500/10 px-2 py-1 rounded-md">
                      #{t}
                    </span>
                  ))}
                </div>
