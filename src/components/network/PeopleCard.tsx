...
function ClaspireBanner() {
  return (
    <div className="absolute inset-0 w-full h-full bg-[#1E1B4B] flex items-center justify-center">
      ...
      <span className="relative text-[15px] lg:text-lg font-extrabold tracking-[0.2em] text-[#0A66C2] bg-clip-text text-transparent select-none">
        CLASPIRE
      </span>
    </div>
  )
...
export default function PeopleCard({ person, onConnect, onRemove, onRespond, onWithdraw, connectionId, showActions = true }: PeopleCardProps) {
  ...
  const scoreColor = matchScore !== null
    ? matchScore >= 90
      ...
      : matchScore >= 70
        ? 'text-[#0A66C2] bg-[#EAF4FF] border-[#0A66C2]/30'
        : matchScore >= 50
      ...
      <div className="px-3 lg:px-4 pb-2.5 lg:pb-3 pt-1 lg:pt-1.5 relative z-[2]">
        ...
        <div className="flex items-center justify-center gap-1.5 mt-1.5 lg:mt-2">
          ...
          {person.mutualConnections > 0 && (
            <span className="hidden lg:inline-flex items-center gap-0.5 text-[10px] font-medium text-[#0A66C2] bg-[#EAF4FF]/80 px-1.5 py-0.5 rounded-full border border-[#0A66C2]/20/60">
              <Users size={10} />
          ...
          <div className="mt-2 lg:mt-2.5 pt-2 lg:pt-2 border-t border-surface dark:border-[#38434F]">
            {localStatus === 'none' && (
              <button
                ...
                disabled={connecting}
                className="w-full h-7 lg:h-8 rounded-lg text-[10px] lg:text-xs font-semibold text-white bg-[#0A66C2] hover:bg-[#004182] flex items-center justify-center gap-1 disabled:opacity-50 transition-all duration-200"
              >
