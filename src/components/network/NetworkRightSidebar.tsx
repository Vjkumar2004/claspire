        ...
        <div className="px-5 pt-5 pb-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-500 to-fuchsia-500 flex items-center justify-center shadow-sm">
                <Sparkles size={13} className="text-white" />
              </div>
              <h3 className="text-sm font-bold text-gray-900">Suggested Mentors</h3>
            </div>
            <button
          ...
          {mentors.length > 0 ? (
            <div className="space-y-1">
              {mentors.slice(0, 4).map((mentor) => (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0 ring-2 ring-gray-100 group-hover:ring-purple-300 group-hover:ring-3 transition-all">
                    {mentor.avatar_url ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-purple-600 transition-colors flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {mentor.college?.short_name && (
                        <span className="text-[10px] text-gray-400 truncate">{mentor.college.short_name}</span>
                      )}
                      {getYearsOfExperience(mentor) && (
                        <>
                          <span className="w-1 h-1 rounded-full bg-gray-300" />
                          <span className="text-[10px] font-medium text-purple-500">{getYearsOfExperience(mentor)}</span>
                        </>
                      )}
                    </div>
                  </div>
                  <ChevronRight size={15} className="text-gray-300 flex-shrink-0 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 group-hover:text-purple-400" />
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-gray-400 py-4 text-center">No mentors available</p>
          )}
        </div>
        <div className="h-px bg-gradient-to-r from-transparent via-gray-200 to-transparent" />
        <button
          onClick={() => router.push('/seniors')}
          className="w-full py-3 text-xs font-semibold text-purple-600 hover:bg-purple-50/50 transition-colors flex items-center justify-center gap-1"
        >
        ...
        {communities.length > 0 ? (
          <div className="space-y-2">
            {communities.slice(0, 4).map((community, index) => {
              const icons = [
                { icon: Brain, gradient: 'from-purple-500 to-fuchsia-500', lightBg: 'bg-purple-100', iconColor: 'text-purple-600' },
                { icon: Rocket, gradient: 'from-emerald-500 to-teal-500', lightBg: 'bg-emerald-100', iconColor: 'text-emerald-600' },
                { icon: Code, gradient: 'from-orange-500 to-red-500', lightBg: 'bg-orange-100', iconColor: 'text-orange-600' },
                { icon: GraduationCap, gradient: 'from-blue-500 to-indigo-500', lightBg: 'bg-blue-100', iconColor: 'text-blue-600' },
              ]
              ...
              return (
                <div
                  ...
                  className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 cursor-pointer transition-all group hover:-translate-y-0.5 hover:shadow-sm duration-200"
                >
                  ...
                  <ChevronRight size={14} className="text-gray-300 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              )
            })}
          </div>
        ) : (
          <p className="text-xs text-gray-400 py-4 text-center">No communities yet</p>
        )}
      </div>
        ...
        <div className="px-5 pt-5 pb-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-gray-900">Network Growth</h3>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2.5 mb-3">
            <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-xl p-3">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">New</span>
              </div>
              <p className="text-xl font-extrabold text-gray-900">{networkGrowth?.newConnections ?? 0}</p>
              <p className="text-[10px] text-gray-500 font-medium">connections this month</p>
            </div>
            <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-xl p-3">
              <div className="flex items-center gap-2 mb-1.5">
              ...
              <p className="text-xl font-extrabold text-gray-900">{connectionsCount}</p>
              <p className="text-[10px] text-gray-500 font-medium">active connections</p>
            </div>
          </div>
            ...
            {allZero ? (
              <div className="flex flex-col items-center justify-center py-4 text-gray-400">
                <svg className="w-full" viewBox={`0 0 ${trendLine.width} ${trendLine.height}`} style={{ height: `${trendLine.height}px` }}>
                  <line x1="0" y1={trendLine.height - 4} x2={trendLine.width} y2={trendLine.height - 4} stroke="#E5E7EB" strokeWidth="1.5" strokeDasharray="4 3" />
                </svg>
              ...
              <svg viewBox={`0 0 ${trendLine.width} ${trendLine.height}`} className="w-full" style={{ height: `${trendLine.height}px` }}>
                ...
                <path d={trendLine.d} fill="none" stroke="#7C3AED" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          ...
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
            ...
            <span className="text-xs font-bold text-gray-700">{networkGrowth?.profileViews ?? 0}</span>
          </div>
        </div>
      </div>
    </div>
