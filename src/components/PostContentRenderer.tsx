...
const convertUrlsToLinks = (html: string) => {
  ...
  const withLinks = withoutAnchors.replace(linkRegex, (match, url, email) => {
    if (url) {
      return '<a href="' + url + '" target="_blank" rel="noopener noreferrer" class="text-[#7C3AED] hover:underline font-medium">' + url + '</a>'
    }
...
export default function PostContentRenderer({ content, clamp }: PostContentRendererProps) {
  ...
  const finalClassName = `text-[13px] sm:text-sm text-slate-700 dark:text-[#CBD5E1] leading-[1.65] font-normal [&>blockquote]:border-l-4 [&>blockquote]:border-[#7C3AED] [&>blockquote]:pl-4 [&>blockquote]:text-slate-500 [&>ul]:list-disc [&>ul]:pl-5 [&>ol]:list-decimal [&>ol]:pl-5 [&>a]:text-[#7C3AED] [&>a]:hover:underline [&_a]:font-medium ${clampClass}`
