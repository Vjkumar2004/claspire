        ...
        <div className="px-4 pb-4 relative flex flex-col items-center -mt-10">
          ...
          <div className="w-20 h-20 rounded-md border-4 border-white overflow-hidden bg-app dark:bg-[#1D2226] shadow-md flex items-center justify-center">
            {user?.avatar_url ? (
              ...
              <span className="text-2xl font-black text-slate-800 dark:text-white uppercase">
                {user?.full_name?.[0] || 'U'}
              </span>
            )}
          </div>
          ...
          {/* Name & Badge details */}
          <h3 className="font-bold text-slate-900 dark:text-white text-sm mt-3 text-center leading-tight">
            {user?.full_name || 'Guest User'}
          </h3>
          <p className="text-[10px] font-semibold text-slate-400 dark:text-[#B0B7BE] mt-0.5 text-center truncate w-full">
          ...
          <div className="flex items-center gap-1 mt-2">
            <span className={`text-[8px] font-extrabold uppercase px-2 py-0.5 rounded-full border ${user?.role === 'senior'
              ...
              : 'bg-[#EAF4FF] text-[#0A66C2] border-[#0A66C2]/20'
              }`}>
          ...
          <div className="w-full pt-3 space-y-2 text-[10px] text-slate-500 dark:text-[#B0B7BE] font-semibold border-b border-surface dark:border-[#38434F] pb-3">
            ...
            {user?.branch && (
              <div className="flex items-center gap-2">
                <Building2 className="w-3.5 h-3.5 text-slate-400 dark:text-[#B0B7BE] flex-shrink-0" />
                <span className="truncate">{user.branch}</span>
              </div>
            )}
            <div className="flex items-center gap-2">
              <Award className="w-3.5 h-3.5 text-slate-400 dark:text-[#B0B7BE] flex-shrink-0" />
          ...
          <div className="w-full pt-3 grid grid-cols-2 gap-2 text-center">
            <div className="p-2 bg-app dark:bg-[#1D2226] rounded border border-surface dark:border-[#38434F]">
              <span className="block text-[14px] font-black text-[#0A66C2] leading-none">
          ...
          {[
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 text-left rounded font-bold text-xs transition-colors cursor-pointer ${isActive ? 'bg-[#EAF4FF] text-[#0A66C2]' : 'text-slate-600 dark:text-[#B0B7BE] hover:text-black dark:hover:text-white hover:bg-app dark:hover:bg-[#1D2226]'
                  }`}
              >
                <item.icon className={`w-3.5 h-3.5 ${isActive ? 'text-[#0A66C2]' : 'text-slate-400 dark:text-[#B0B7BE]'}`} />
