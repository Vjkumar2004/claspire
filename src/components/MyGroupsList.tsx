...
export default function MyGroupsList() {
  ...
  if (loading) return (
    <div className="bg-surface dark:bg-[#283036] rounded-2xl p-6 shadow-sm dark:shadow-[#1D2226]/50">
  ...
  if (error) return (
    <div className="bg-surface dark:bg-[#283036] rounded-2xl p-6 shadow-sm dark:shadow-[#1D2226]/50 text-center py-8">
      ...
      <button onClick={fetchGroups} className="px-4 py-2 bg-[#0A66C2] text-white rounded-lg text-sm">Try Again</button>
    </div>
  )
  ...
  if (groups.length === 0) return (
    <div className="bg-surface dark:bg-[#283036] rounded-2xl p-6 shadow-sm dark:shadow-[#1D2226]/50 text-center py-8">
      ...
      <p className="text-gray-500 dark:text-[#B0B7BE] text-sm">You haven't created any student groups yet.</p>
    </div>
  )

  return (
    <div className="bg-surface dark:bg-[#283036] rounded-2xl p-4 sm:p-6 shadow-sm dark:shadow-[#1D2226]/50">
      <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-6">My Student Groups</h2>
      ...
      <div className="space-y-3 sm:space-y-4">
        {groups.map((group, index) => (
          <motion.div
            ...
            className="bg-gray-50 border border-surface dark:border-[#38434F] rounded-xl sm:rounded-2xl p-4 sm:p-5 hover:shadow-lg transition-all hover:border-[#0A66C2]/30"
          >
            <div className="flex flex-col sm:flex-row sm:items-start gap-4">
              {/* Creator */}
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="w-12 h-12 rounded-full bg-[#0A66C2] flex items-center justify-center text-white font-bold overflow-hidden flex-shrink-0">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs bg-[#EAF4FF] text-[#004182] px-2 py-0.5 rounded-full font-medium">Admin</span>
                  </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-gray-900 dark:text-white text-base mb-1 truncate">{group.name}</h3>
                    <p className="text-gray-600 dark:text-[#B0B7BE] text-sm line-clamp-2">{group.description || 'No description provided'}</p>
                  </div>
        ...
        {showRequestsModal && selectedGroup && (
          ...
          >
            <motion.div
              ...
              className="bg-surface dark:bg-[#283036] rounded-2xl w-full max-w-lg max-h-[80vh] overflow-hidden shadow-2xl"
              onClick={e => e.stopPropagation()}
            >
              <div className="bg-[#0A66C2] p-5 text-white">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="p-2 hover:bg-surface dark:bg-[#283036]/20 rounded-lg transition-colors disabled:opacity-50"
                      title="Refresh Requests"
                    >
                      <RefreshCw size={18} className={loadingRequests ? 'animate-spin' : ''} />
                    </button>
                    <button onClick={() => setShowRequestsModal(false)} className="p-2 hover:bg-surface dark:bg-[#283036]/20 rounded-lg transition-colors">
                      <X size={18} />
                    </button>
                  </div>
                </div>
              ...
              <div className="p-5 overflow-y-auto max-h-[60vh]">
                {loadingRequests ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 dark:text-[#B0B7BE] font-medium">No pending requests</p>
                    <p className="text-gray-400 dark:text-[#B0B7BE] text-sm mt-1">All caught up!</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {requests.map((req) => (
                      <div key={req.id} className="border border-surface dark:border-[#38434F] rounded-xl p-4 hover:border-amber-200 transition-colors">
                        <div className="flex items-start gap-3">
                          <div className="w-10 h-10 rounded-full bg-[#0A66C2] flex items-center justify-center text-white font-bold text-sm overflow-hidden flex-shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-bold text-gray-900 dark:text-white text-sm">{req.users?.full_name}</span>
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                req.users?.role === 'senior' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'
                              }`}>{req.users?.role}</span>
                            </div>
                            <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-[#B0B7BE] flex-wrap">
                            ...
                            <div className="flex items-center gap-1 mt-1 text-xs text-gray-400 dark:text-[#B0B7BE]">
                              <Clock size={10} />
        ...
        {showManageModal && selectedGroup && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-surface dark:bg-[#283036] rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden shadow-2xl"
              onClick={e => e.stopPropagation()}
            >
              <div className="bg-[#0A66C2] p-5 text-white flex items-center justify-between">
                ...
                <button onClick={() => setShowManageModal(false)} className="p-2 hover:bg-surface dark:bg-[#283036]/20 rounded-lg transition-colors">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="p-5 overflow-y-auto max-h-[60vh]">
                {loadingMembers ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 dark:text-[#B0B7BE]">No members found</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {members.map((member) => (
                      <div key={member.id} className={`flex items-center gap-3 p-4 rounded-xl border ${member.is_blocked ? 'bg-red-50 border-red-200' : 'bg-app border-surface dark:border-[#38434F]'}`}>
                        <div className="w-10 h-10 rounded-full bg-[#0A66C2] flex items-center justify-center text-white font-bold text-sm overflow-hidden flex-shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <span className="font-semibold text-gray-900 dark:text-white text-sm truncate">{member.user.full_name}</span>
                          ...
                          <p className="text-xs text-gray-500 dark:text-[#B0B7BE]">{member.user.unique_id} • Joined {formatDate(member.joined_at)}</p>
                        </div>
                        {member.role !== 'admin' && (
                          <button
