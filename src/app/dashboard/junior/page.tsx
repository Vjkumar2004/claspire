...
export default function JuniorDashboard() {
  ...
  const getRPLevel = (points: number) => {
    if (points >= 5000) return { label: 'Legend', emoji: '👑', next: null, color: '#0A66C2' }
    if (points >= 1500) return { label: 'Champion', emoji: '🏆', next: 5000, color: '#0A66C2' }
    ...
    return { label: 'Explorer', emoji: '🌱', next: 100, color: '#0A66C2' }
  }
      ...
      <aside className={`fixed top-0 left-0 h-full w-[280px] bg-surface dark:bg-[#283036] border-r border-surface/60 dark:border-[#38434F] z-[101] flex flex-col justify-between transition-all duration-300 transform lg:translate-x-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-sm font-bold transition-all group cursor-pointer ${activeTab === item.id
                    ? 'bg-[#F5F3FF] text-[#0A66C2] shadow-sm shadow-[#0A66C2]/5'
              ...
              >
                <span className={`transition-transform duration-300 ${activeTab === item.id ? 'scale-110 text-[#0A66C2]' : 'text-slate-400 dark:text-[#B0B7BE] group-hover:scale-110 group-hover:text-slate-600 dark:text-[#B0B7BE] dark:group-hover:text-[#B0B7BE] dark:hover:text-[#B0B7BE]'}`}>
                  {item.icon}
                </span>
                {item.label}
                {activeTab === item.id && (
                  <motion.div layoutId="nav-pill" className="ml-auto w-1.5 h-1.5 rounded-full bg-[#0A66C2]" />
                )}
              </button>
            ))}
          </nav>
        ...
        <div className="p-6">
          <div className="bg-slate-900 rounded-3xl p-5 text-white shadow-xl relative overflow-hidden group border border-white/5">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#0A66C2]/10 rounded-full -mr-16 -mt-16 blur-2xl group-hover:bg-[#0A66C2]/20 transition-colors" />
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-4">
                ...
                <Flame size={16} className="text-[#0A66C2] animate-pulse" />
              </div>
              <div className="text-3xl font-bold mb-1 text-white tracking-tight">{u.rise_points}</div>
              <div className="text-[10px] font-extrabold text-[#0A66C2]/60 mb-4 flex items-center gap-1.5 bg-surface/5 w-fit px-2 py-0.5 rounded-full border border-white/5">
                {rp.emoji} {rp.label}
              </div>
              <div className="space-y-1.5">
                <div className="flex justify-between text-[9px] font-bold text-slate-400 dark:text-[#B0B7BE]">
                  <span>Level Progress</span>
                  <span className="text-[#0A66C2]/60">{Math.round(rpProgress)}%</span>
                </div>
      ...
      <main className="lg:ml-[280px] min-h-screen pb-20 min-w-0">
        ...
        <div className="lg:hidden sticky top-0 bg-surface/85 backdrop-blur-md border-b border-surface/60 dark:border-[#38434F] z-30 px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <NotificationBell />
            <div className={`w-8 h-8 rounded-xl ${u.avatar_url ? 'bg-transparent' : 'bg-[#0A66C2]'} flex items-center justify-center text-white text-xs font-bold overflow-hidden border border-surface/60 dark:border-[#38434F] shadow-sm`}>
              {u.avatar_url ? (
...
<div className="flex flex-col md:flex-row items-center md:items-end gap-5 -mt-8 md:-mt-10 relative z-10 text-center md:text-left">                {/* Avatar with White Border */}
                <div className="w-28 h-28 md:w-32 md:h-32 rounded-3xl bg-surface dark:bg-[#283036] p-1 shadow-md flex items-center justify-center overflow-hidden flex-shrink-0">
                  <div className={`w-full h-full rounded-2xl ${u.avatar_url ? 'bg-transparent' : 'bg-[#0A66C2]'} flex items-center justify-center text-white text-3xl font-bold overflow-hidden`}>
                    {u.avatar_url ? (
                      <img src={u.avatar_url} alt={u.full_name} className="w-full h-full object-cover" />
                    ) : (
                      u.full_name[0].toUpperCase()
                ...
                <div className="pt-2 md:pt-0">
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-2">
                    ...
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-[#EAF4FF] border border-[#0A66C2]/20 text-[#0A66C2] text-[9px] font-extrabold tracking-wider uppercase">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <AnimatePresence mode="wait">
                {activeTab === 'overview' && (
                  ...
                  >
                    ...
                    <div className="bg-surface dark:bg-[#283036] rounded-3xl border border-surface/60 dark:border-[#38434F] p-6 shadow-sm">
                      <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-2.5">
                          <div className="w-9 h-9 rounded-xl bg-[#EAF4FF] flex items-center justify-center text-[#0A66C2]">
                            <Users size={16} />
                          </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {[
                        { label: 'Doubts Asked', value: u.doubt_count, icon: <HelpCircle size={20} />, trend: '+1 this week', bg: 'from-[#0A66C2]/5 to-indigo-500/5', border: 'hover:border-[#0A66C2]/30', iconColor: 'text-[#0A66C2] bg-[#EAF4FF]' },
                        { label: 'Active Referrals', value: dashData.myReferrals.length, icon: <Handshake size={20} />, trend: 'Direct Tracking', bg: 'from-cyan-500/5 to-blue-500/5', border: 'hover:border-[#D9E2EC]', iconColor: 'text-cyan-600 bg-cyan-50' },
                      ...
                      <div className="flex gap-1 bg-app dark:bg-[#1D2226] border border-surface dark:border-[#38434F] p-1 rounded-xl self-start sm:self-auto">
                        {['all', 'answered', 'pending'].map(f => (
                          <button
                            ...
                            className={`px-3.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all capitalize cursor-pointer ${doubtFilter === f ? 'bg-surface dark:bg-[#283036] shadow-sm text-[#0A66C2]' : 'text-slate-400 dark:text-[#B0B7BE] hover:text-slate-600 dark:text-[#B0B7BE] dark:hover:text-[#B0B7BE]'}`}
                          >
                            {f}
                          </button>
                        ))}
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {(() => {
                        ...
                        return filteredPosts.map(post => (
                          <div key={post.id} className="group bg-surface dark:bg-[#283036] p-6 rounded-3xl border border-surface/60 dark:border-[#38434F] hover:border-[#0A66C2]/30 transition-all shadow-sm hover:shadow-md relative overflow-hidden cursor-pointer">
                            <div className="flex justify-between items-center mb-4">
                              ...
                              <div className="flex gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-app dark:bg-[#1D2226] text-slate-500 dark:text-[#B0B7BE] hover:bg-[#EAF4FF] hover:text-[#0A66C2] transition-all shadow-sm border border-surface/60"
                                  title="Edit Post"
                                >
                                  <Pencil size={12} />
                                </button>
                            ...
                            <div className="flex gap-4">
                              <div className={`w-9 h-9 rounded-xl ${post.users?.avatar_url ? 'bg-transparent' : 'bg-[#EAF4FF]'} flex items-center justify-center text-[#0A66C2] font-bold text-xs overflow-hidden flex-shrink-0 mt-0.5 border border-surface/60`}>
                              ...
                              <div className="flex-1">
                                <h3 className="text-base font-bold text-[#0F172A] dark:text-white group-hover:text-[#0A66C2] transition-colors mb-1.5 tracking-tight leading-snug">{post.title}</h3>
                      ...
                      <div key={i} className="bg-surface dark:bg-[#283036] rounded-3xl border border-surface/60 dark:border-[#38434F] overflow-hidden group shadow-sm hover:shadow-md transition-all flex flex-col justify-between cursor-pointer">
                        <div>
                          <div className="h-40 bg-[#0F172A] relative overflow-hidden">
                            <div className="absolute inset-0 bg-[#EAF4FF]/50" />
                          ...
                          <div className="p-6">
                            <h3 className="text-base font-bold text-[#0F172A] dark:text-white m-0 mb-1.5 truncate group-hover:text-[#0A66C2] transition-colors leading-snug tracking-tight">{w.title}</h3>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p
                                  onClick={() => router.push(`/u/${w.users.unique_id}`)}
                                  className="text-xs font-bold text-[#0F172A] dark:text-white m-0 cursor-pointer hover:text-[#0A66C2] transition-colors"
                                >
                                  {w.users.full_name}
                                </p>
                ...
                {activeTab === 'community' && (
                  <motion.div key="community" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                    ...
                    <div className="bg-[#0A66C2] rounded-3xl p-8 text-white relative overflow-hidden shadow-md">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-4 px-2">
                        ...
                        <button
                          onClick={() => setShowCreateGroupModal(true)}
                          className="px-3.5 py-1.5 bg-[#EAF4FF] text-[#0A66C2] border border-[#0A66C2]/20 rounded-xl font-bold text-[10px] hover:bg-[#EAF4FF] transition-colors flex items-center gap-1.5"
                        >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {dashData.joinedCommunities.map((item, i) => (
                          <div key={i} className="bg-surface dark:bg-[#283036] p-5 rounded-3xl border border-surface/60 dark:border-[#38434F] flex items-center justify-between group hover:shadow-md transition-all cursor-pointer">
                            <div className="flex items-center gap-4">
                              <div className="w-11 h-11 rounded-xl bg-[#EAF4FF] flex items-center justify-center text-[#0A66C2] font-extrabold text-sm border border-[#0A66C2]/20 group-hover:scale-105 transition-transform duration-300">
                            ...
                            <button onClick={() => router.push(`/community/c/${item.communities.slug}`)} className="p-2.5 rounded-xl bg-app dark:bg-[#1D2226] text-slate-400 dark:text-[#B0B7BE] group-hover:bg-[#EAF4FF] group-hover:text-[#0A66C2] transition-colors cursor-pointer border border-transparent group-hover:border-[#0A66C2]/20">
                  ...
                  <motion.div key="referrals" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    ...
                    <div className="space-y-4">
                      {dashData.myReferrals.length > 0 ? dashData.myReferrals.map((req, i) => (
                        <div key={i} className="bg-surface dark:bg-[#283036] p-5 rounded-3xl border border-surface/60 dark:border-[#38434F] hover:border-[#0A66C2]/30 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group cursor-pointer shadow-sm">
                          <div className="flex items-center gap-4">
                            <div
            ...
            <div className="lg:col-span-4 space-y-8">
              ...
              <div className="bg-surface dark:bg-[#283036] rounded-3xl border border-surface/60 dark:border-[#38434F] p-6 shadow-sm overflow-hidden relative group">
                <div className="absolute top-0 right-0 w-24 h-24 bg-[#EAF4FF] group-hover:bg-[#EAF4FF] transition-colors rounded-bl-full -mr-8 -mt-8" />
                ...
                <div className="space-y-3 relative z-10">
                  {[
                    ...
                    <div key={i} className={`flex items-center justify-between p-3.5 rounded-2xl border transition-all cursor-pointer ${task.done ? 'bg-green-50/50 border-green-100/50' : 'bg-app dark:bg-[#1D2226] border-transparent hover:bg-surface dark:hover:bg-[#283036] hover:border-surface dark:hover:border-[#38434F] shadow-sm'}`}>
                      ...
                      <span className={`text-[10px] font-black ${task.done ? 'text-green-600' : 'text-[#0A66C2]'}`}>{task.rp} RP</span>
                    </div>
                  ))}
                </div>
              </div>
