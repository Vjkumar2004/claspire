...
function CommunityPageContent() {
  ...
  const enrichCommunitiesWithLiveCounts = async (list: any[]) => {
    ...
    try {
      ...
      const countsBySlug = new Map(data.communities.map((c: any) => [c.slug, c.member_count || 0]))
      const seniorsBySlug = new Map(data.communities.map((c: any) => [c.slug, c.senior_count || 0]))

      return [...list].map((c) => ({
        ...c,
        member_count: Math.max(c.member_count || 0, countsBySlug.get(c.slug) || 0),
        senior_count: Math.max(c.senior_count || 0, seniorsBySlug.get(c.slug) || 0),
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-plus-jakarta-sans text-[#1E293B] antialiased pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 text-left rounded font-bold text-xs transition-colors cursor-pointer ${
                        isActive ? 'bg-purple-50 text-[#7C3AED]' : 'text-slate-600 hover:text-black hover:bg-slate-50'
                      }`}
                    >
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded transition-all font-bold text-[11px] whitespace-nowrap cursor-pointer ${
                        isActive 
                            ...
                            <div>
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className={`text-[7px] font-black uppercase px-1 rounded ${
                                  post.users?.role === 'senior' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-50 text-slate-600 border border-slate-100'
                                }`}>
                            ...
                            <div className="flex items-center bg-slate-50 border border-slate-200 rounded p-0.5">
                              ...
                              <button
                                ...
                                className={`flex items-center px-1.5 py-0.5 rounded transition-all cursor-pointer ${
                                  votes[post.id]?.userVote === 'downvote' 
          ...
          <aside className="lg:col-span-3 sticky top-[88px] self-start space-y-4 hidden lg:block">
            ...
            {/* Active Placements / Jobs list (LinkedIn placement prep feel) */}
            <div className="bg-white rounded-md border border-slate-200 p-3.5 shadow-sm">
              <h4 className="font-bold text-slate-900 text-xs mb-3 flex items-center gap-1.5">
              ...
              <div className="space-y-2.5 text-[10px] font-semibold text-slate-600">
                <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                  <div>
                    <p className="font-bold text-slate-800">Software Developer Intern</p>
                    <p className="text-slate-400 mt-0.5 text-[9px]">Google India • Referral hiring</p>
                  </div>
                  <button 
                    onClick={() => router.push('/jobs')}
                    className="px-2 py-0.5 bg-rose-50 border border-rose-100 hover:bg-rose-100 text-rose-600 rounded font-bold text-[9px] cursor-pointer"
                  >
                    Apply
                  </button>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-bold text-slate-800">Systems Engineer</p>
                    <p className="text-slate-400 mt-0.5 text-[9px]">TCS Campus Prep • Mock interview</p>
                  </div>
                  <button 
                    onClick={() => router.push('/jobs')}
                    className="px-2 py-0.5 bg-slate-50 border border-slate-100 hover:bg-slate-100 text-slate-600 rounded font-bold text-[9px] cursor-pointer"
                  >
                    View
                  </button>
                </div>
                        ...
                        drawerMessages.map((msg: any) => {
                          ...
                          return (
                            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                              <div className={`max-w-[85%] p-2.5 rounded-xl text-[10.5px] font-semibold ${
                                isMine 
      ...
      <div 
        className={`fixed bottom-0 right-6 z-[9999] w-80 bg-white rounded-t-xl shadow-[0_-4px_25px_rgba(0,0,0,0.12)] border border-slate-200 overflow-hidden font-plus-jakarta-sans transition-all duration-300 hidden lg:block ${
          chatExpanded ? 'h-[440px]' : 'h-11'
        }`}
      >
        {/* Header */}
        <div 
                ...
                >
                  {drawerChatLoading && drawerMessages.length === 0 ? (
                    ...
                    drawerMessages.map((msg: any) => {
                      ...
                      return (
                        <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[85%] p-2 rounded-lg text-[10px] font-semibold ${
                            isMine 
                              ? `bg-purple-600 text-white rounded-br-none ${isOptimistic ? 'opacity-70' : ''}` 
                              : 'bg-white border border-slate-200 text-slate-800 rounded-bl-none shadow-sm'
                          }`}>
