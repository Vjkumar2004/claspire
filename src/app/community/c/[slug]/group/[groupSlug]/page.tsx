...
export default function GroupChatPage() {
  ...
  const chatContent = loading ? (
    <div className="flex-1 flex items-center justify-center bg-[#efeae2] dark:bg-[#1D2226]">
      <div className="w-8 h-8 border-4 border-[#0A66C2]/20 border-t-purple-600 rounded-full animate-spin" />
    </div>
  ) : !groupData ? null : (
    <>
      {/* Header */}
      <div className="flex-shrink-0 bg-surface dark:bg-[#283036] border-b border-surface dark:border-[#38434F] px-3 py-2 sm:px-4 sm:py-3 flex items-center justify-between z-40 shadow-sm min-h-[60px]">
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          ...
          <div className="w-10 h-10 rounded-full bg-[#EAF4FF] text-[#0A66C2] flex items-center justify-center font-bold text-base overflow-hidden flex-shrink-0">
        ...
        <div className="flex-shrink-0 pl-2">
          <button onClick={() => setShowMembers(!showMembers)} className="p-2 hover:bg-surface-hover dark:bg-[#283036] dark:hover:bg-[#1D2226] rounded-full transition-colors relative">
            ...
            <span className="absolute top-0 right-0 w-4 h-4 bg-[#0A66C2] text-white text-[10px] rounded-full flex items-center justify-center font-bold shadow-sm border border-white">
              ...
              {groupData.members.map((member) => (
                <div key={member.id} className="px-4 py-3 hover:bg-surface-hover dark:bg-[#283036] dark:hover:bg-[#1D2226] transition-colors flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm overflow-hidden flex-shrink-0 ${
                    member.avatar_url ? 'bg-transparent shadow-sm' : member.role === 'senior' ? 'bg-[#0A66C2]' : 'bg-[#0A66C2]'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {member.membership_role === 'admin' && (
                        <span className="text-[10px] bg-[#EAF4FF] text-[#0A66C2] px-2 py-0.5 rounded-full font-bold flex-shrink-0">Admin</span>
                      )}
                    </div>
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 opacity-60">
            <div className="w-12 h-12 bg-surface dark:bg-[#283036] rounded-full flex items-center justify-center shadow-sm">
              <Sparkles size={24} className="text-[#0A66C2]" />
            </div>
              ...
              <div key={message.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex gap-2 max-w-[85%] sm:max-w-[70%] ${isOwn ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className="w-8 h-8 flex-shrink-0 mt-auto">
                      {showAvatar ? (
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs overflow-hidden shadow-sm ${
                          message.sender?.avatar_url ? 'bg-transparent' : message.sender?.role === 'senior' ? 'bg-amber-500' : 'bg-[#0A66C2]'
                        }`}>
                  ...
                  <div className="flex flex-col gap-0.5 relative">
                    {!isOwn && showAvatar && (
                      <span className="text-[11px] font-semibold text-[#0A66C2] px-1 ml-1">{message.sender?.full_name}</span>
                    )}
                    <div className={`px-4 py-2 text-sm leading-relaxed shadow-sm ${
                      isOwn
                        ? 'bg-[#0A66C2] text-white rounded-2xl rounded-br-none'
  ...
  <form onSubmit={handleSendMessage} className="flex gap-2 items-center">
    <input
      ...
      className="flex-1 bg-surface dark:bg-[#283036] border border-surface dark:border-[#38434F] text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-[#B0B7BE] rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-[#0A66C2] focus:ring-1 focus:ring-[#0A66C2] transition-colors disabled:opacity-50 shadow-sm"
    />
    <button
      type="submit"
      disabled={!newMessage.trim() || sending || !groupData.canMessage}
      className="p-3 flex-shrink-0 bg-[#0A66C2] hover:bg-[#004182] text-white rounded-2xl flex items-center justify-center transition-colors disabled:opacity-50 active:scale-95 shadow-sm"
    >
      {sending ? (
    ...
    <button onClick={handleJoin} disabled={joining} className="w-full bg-[#0A66C2] hover:bg-[#004182] text-white px-6 py-3 rounded-2xl font-bold text-sm transition-all active:scale-[0.98] disabled:opacity-50 shadow-sm shadow-[#0A66C2]/20">
      ...
      <AnimatePresence>
        {showTermsModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center p-4" onClick={() => setShowTermsModal(false)}>
            ...
            >
              <div className="bg-[#EAF4FF] p-5 border-b border-[#0A66C2]/20">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-[#EAF4FF] rounded-full flex items-center justify-center text-[#0A66C2]">
                    <Users size={20} />
                  </div>
                  <div>
              ...
              <div className="p-5 space-y-3 max-h-[50vh] overflow-y-auto">
                {['Be respectful and professional', 'Share relevant content only', 'No spam or self-promotion', 'Protect member privacy', 'Report inappropriate content'].map((rule, i) => (
                  <div key={i} className="flex items-center gap-3 text-sm text-gray-600 dark:text-[#B0B7BE]">
                    <span className="w-5 h-5 bg-[#EAF4FF] text-[#0A66C2] rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">✓</span>
                    {rule}
                  </div>
                ))}
                ...
                <label className="flex items-start gap-3 cursor-pointer mt-4">
                  <input 
                    type="checkbox" 
                    className="mt-0.5 w-4 h-4 rounded border-gray-300 text-[#0A66C2] focus:ring-[#0A66C2]" 
              ...
              <div className="p-4 bg-app dark:bg-[#1D2226] border-t border-surface dark:border-[#38434F] flex gap-3">
                ...
                <button 
                  ...
                  className={`flex-1 py-2.5 rounded-2xl text-white text-sm font-bold transition-all shadow-sm ${
                    termsAccepted 
                      ? 'bg-[#0A66C2] hover:bg-[#004182] active:scale-95' 
