      ...
      <div className="max-w-4xl mx-auto px-6 py-20">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-sm font-semibold text-[#0A66C2] uppercase tracking-widest m-0">Claspire Blog</p>
          </div>
          ...
          <div className="space-y-6">
            {posts.map(post => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="block group no-underline">
                <div className="bg-surface dark:bg-[#283036] border border-surface dark:border-[#38434F] rounded-2xl p-8 hover:border-[#0A66C2]/30 hover:shadow-md transition-all duration-200">
                  ...
                  <h2 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-[#0A66C2] transition-colors mb-2">{post.title}</h2>
                  <p className="text-gray-500 dark:text-[#B0B7BE] leading-relaxed text-[15px]">{post.description}</p>
                  <span className="inline-block mt-4 text-sm text-[#0A66C2] font-semibold group-hover:translate-x-1 transition-transform">
                    Read article →
                  </span>
                </div>
              </Link>
